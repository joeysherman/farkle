...
export const RoomControls: React.FC<{
	...
	onShowInvite: () => void;
	onPlayBotTurns: () => Promise<void>;
}> = ({ room, user, onStartGame, onEndGame, onShowInvite, onPlayBotTurns }) => {
	...
	return (
		<div className="flex flex-row gap-2 w-full">
			...
			{room?.status === "in_progress" && (
				<>
					<button
						className="flex-1 inline-flex justify-center items-center gap-2 px-4 py-2.5 sm:py-2 border border-transparent text-base sm:text-sm font-medium rounded-xl sm:rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
						onClick={onPlayBotTurns}
					>
						<svg
							className="h-5 w-5 sm:h-4 sm:w-4"
							fill="currentColor"
